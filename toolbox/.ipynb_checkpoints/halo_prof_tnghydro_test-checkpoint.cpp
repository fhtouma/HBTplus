...
struct HaloSize_t
{
    ...
    HaloSize_t(){};
    void Compute(HBTxyz &cen, HBTxyz &vel_cen, HBTReal &rvir, ParticleSnapshot_t &snap, Linkedlist_t &ll, HBTReal scalefactor, HBTReal hz);
    void Compute(HBTxyz &cen, HBTxyz &vel_cen, HBTReal &rvir, ParticleSnapshot_t &snap, GeoTree_t &tree, HBTReal scalefactor, HBTReal hz);
};
...
int main(int argc, char **argv)
{
    ...
    for(int grpid=0;grpid<10;grpid+=1){
        cout<<HaloSize[grpid].GroupIndex<<", "
            ...
            <<halosnap.Halos[HaloSize[grpid].GroupIndex].PhysicalAverageVelocity[0]<<", "
            <<halosnap.Halos[HaloSize[grpid].GroupIndex].RVir<<endl;
    }
    cout<<endl;
  ...
  #endif
    timer.Tick();
    cout<<", built in "<<timer.GetSeconds()<<"s"<<endl;
    ...
    for(int grpid=0;grpid<HaloSize.size();grpid++)
    {
        ...
        HaloSize[grpid].Compute(halosnap.Halos[grpid].ComovingAveragePosition, halosnap.Halos[grpid].PhysicalAverageVelocity, halosnap.Halos[grpid].RVir, partsnap, searcher, Cosmology.ScaleFactor, Cosmology.Hz);
        if(grpid%100000==0){
...
public:
    ...
    LogBinCollector_t(int (*n)[TypeMax], HBTReal (*vr)[TypeMax], HBTReal (*mfr)[TypeMax], HBTxyz cen, HBTxyz vel_cen, HBTReal rvir, ParticleSnapshot_t &snap, HBTReal scalefactor, HBTReal hz): N(n), VR(vr), MFR(mfr), CEN(cen), VEL_CEN(vel_cen), RVIR(rvir), SNAP(snap), SCALEFACTOR(scalefactor), HZ(hz)
    {}
    void Collect(HBTInt index, HBTReal d2)
    {
        ...
        dy = NEAREST(dy);
        dz = NEAREST(dz);
        HBTReal r_onepart = sqrt(dx*dx + dy*dy + dz*dz); //comoving
        
        if(r_onepart < RMAX*RVIR){
            int ibin=ceilf(logf(r_onepart/RMIN/RVIR)/DlnX);
            if(ibin<0) ibin=0;
            
            HBTReal dvx = partVel[0] - VEL_CEN[0];
            HBTReal dvy = partVel[1] - VEL_CEN[1];
            HBTReal dvz = partVel[2] - VEL_CEN[2];
            HBTReal vr_onepart = (dvx*dx + dvy*dy + dvz*dz)/r_onepart + HZ*r_onepart*SCALEFACTOR; //phy
...
void HaloSize_t::Compute(HBTxyz &cen, HBTxyz &vel_cen, HBTReal &rvir, ParticleSnapshot_t &snap, Linkedlist_t &ll, HBTReal scalefactor, HBTReal hz)
{
    LogBinCollector_t collector(n, vr, mfr, cen, vel_cen, rvir, snap, scalefactor, hz);
    ll.SearchSphere(1.001*RMAX*rvir, cen, collector);
}

void HaloSize_t::Compute(HBTxyz &cen, HBTxyz &vel_cen, HBTReal &rvir, ParticleSnapshot_t &snap, GeoTree_t &tree, HBTReal scalefactor, HBTReal hz)
{
    LogBinCollector_t collector(n, vr, mfr, cen, vel_cen, rvir, snap, scalefactor, hz);
    tree.Search(cen, 1.001*RMAX*rvir, collector);
}
...
void save(vector <HaloSize_t> &HaloSize, int isnap, int ifile, int nfiles)
{
    ...
    if(ifile==0&&nfiles==0)
        filename=filename+"/haloprof_re_"+to_string(isnap)+".hdf5";
    else
        filename=filename+"/haloprof_re_"+to_string(isnap)+"."+to_string(ifile)+".hdf5";
